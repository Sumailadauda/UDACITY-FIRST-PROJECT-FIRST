...
def venues():
  ...
  for venue in venues:
    city_state = Venue.query.order_by(db.desc(Venue.id)).filter_by(state=venue.state).filter_by(city=venue.city).all()
    venue_info = []
...
def search_venues():
  ...
  result = db.session.query(Venue).filter(Venue.name.ilike(f'%{searchTerm}%')).all()
  results = []
...
def create_venue_submission():
  ...
  try:
    ...
    db.session.rollback()
    flash('Venue ' + request.form['name'] + ' could not be listed!', 'danger')
  finally:
    db.session.close()
  return render_template('pages/home.html')
...
def artists():
  ...
  artist=Artist.query.order_by(db.desc(Artist.id)).all()
...
def search_artists():
  ...
  searchTerm = request.form.get('search_term')
  result = db.session.query(Artist).filter(Artist.name.ilike(f'%{searchTerm}%')).all()
  results = []
...
def shows():
  ...
  # TODO: replace with real venues data.
  shows = Show.query.join(Venue).filter(Venue.id == Show.venue_id).join(Artist).filter(Artist.id == Show.artist_id).order_by(db.desc(Show.start_time)).all()
  data=[]
  for show in shows:
    data.append({
      "venue_id": show.venue_id,
